...
# Copy of the gluemncbig file here:
# MITgcm/utils/python/MITgcmutils/scripts/gluemncbig
# Status: 2024-10-29
# Commit: c9bf163375728f2354632935fc69353437506222 of 2024-10-28
# Last commit affecting this script: 9a1526c006765e9da989b686a9ce93d5ab91a202 of 2021-01-11
#
# Note: The command line arguments were removed to be able to call this
# from inside another script. For this, the __main__ section was replaced
# by a function definition.
# Also, all sys.exit() calls were replaced yb throwing exceptions that can
# be caught by whoever calls the function.
# TODO: Add a new __main__ section that catches the exceptions and then
# calls sys.exit().
...
def glue(outname, fnames, version, verbose=False, progress=True, manytiles=False):
    ...
    # The command line arguments would be the one passed to the initial script that
    ...
    if not fnames:
        raise ValueError('You need to specify at least one input file.')
    ...
    if outname is None:
        raise ValueError('You need to specify an output file.')
    ...
    if len(files0) and len(filess):
        raise ValueError('Both files with and without iteration number present.  Aborting.')
    elif len(filess):
        files0 = filess[0]
    else:
        filess = [files0]
    ...
    try:
        ...
        if version == 1:
            for sz in ncout._begins.values():
                if sz >= 1<<31:
                    raise ValueError('ERROR: Variables too big for NetCDF version 1, try "-2" option.')
        ...
        if manytiles:
            ...
            for fnames in filess:
                ...
                for fname in fnames:
                    ...
                    if m is None:
                        raise ValueError('Could not extract tile number from file name: {0}'.format(fname))
                    tn = int(m.group(1)) - 1
                    bj,bi = divmod(tn, ntx)
                    ...
                    for fname in tileslices[bj]:
                        try:
                            ...
                            if e.errno == errno.EMFILE:
                                raise ValueError('ERROR: Too many open files.  Ask developer to improve script ;)')
                            raise
                        ncs.append(nc)
            ...
            for fname in files0[1:]:
                try:
                    ...
                    if e.errno == errno.EMFILE:
                        raise ValueError('ERROR: Too many open files.  Try again with --many or increase the limit on open files.')
                    raise
                ncs.append(nc)
